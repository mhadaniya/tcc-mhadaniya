...
\section{Intro - Web Mining}
		...
		Como a\textit{Web Mining} foi concebida devido aos estudos nas mais diversas linhas de pesquisa: extração de informação, inteligência artificial, banco de dados, recuperação de informação e entre outras áreas. Ela faz parte de um todo, que abrange os problemas acimas citados.  
  
\section{\textit{Web Mining}}
	
		\textit{Web Mining} é o uso das técnicas de Mineração de Dados para descobrir e extrair automaticamente a informação de documentos na Web\cite{Etzione1996}. Seguindo o conceito de Etzione, que utiliza da KDD(Knowledge Discovery Database), ele decompõe a Web Mining em 4 tarefas:
		
		\begin{enumerate}
          \item Resource finding(Coleta de Documentos);
          \item Information selection and pre-processing(Pré-processamento);
          \item Generalization(Extração de Padrões);
          \item Analysis(Análise).
        \end{enumerate}
		
		É imporante ressaltar que 
		 
		A Mineração de Dados refere-se ao processo não trivial de identificação de padrões válidos, previamente desconhecidos e potencialmente úteis de dados.\cite{Frawley1992} 
